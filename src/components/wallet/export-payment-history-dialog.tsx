...
/**
 * Export Laporan Pembayaran Paket, data akun sendiri
 * (docs/be-fe/export-laporan-mutasi-saldo.md; nama di BE: "Laporan Mutasi Saldo").
 * Isinya riwayat pembayaran, bukan mutasi wallet.
 */
export default function ExportPaymentHistoryDialog({
  open,
  onOpenChange,
  initialStatus,
  initialSearch = "",
  ...
  useEffect(() => {
    ...
    setStatus(
      STATUSES.some((s) => s.value === initialStatus)
        ? (initialStatus as string)
        : "all"
    );
    setSearch(initialSearch);
  }, [open, initialStatus, initialSearch, initialDateFrom, initialDateTo]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto rounded-2xl border-slate-100 sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <DialogTitle>Export Laporan Pembayaran Paket</DialogTitle>
              <DialogDescription>
                Berisi riwayat pembayaran akun Anda sendiri. Kosongkan filter
        ...
        <div className="space-y-2">
          <Label
            htmlFor="export-payment-search"
            className="text-sm font-medium text-slate-700"
          >
            Pencarian (opsional)
          </Label>
          <Input
            id="export-payment-search"
            value={search}
            maxLength={255}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Referensi, invoice, metode, atau kanal"
            className={fieldCls}
          />
        </div>
