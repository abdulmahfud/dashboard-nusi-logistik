...
interface Props {
  ...
  /** Status verifikasi awal ("all" | "pending" | "approved" | "rejected"). */
  initialStatus?: string;
}

const fieldCls = "h-11 rounded-lg border-slate-200 bg-white";

/** Export Semua Rekening Bank (docs/be-fe/export-rekening-bank.md). Tanggal = tanggal rekening ditambahkan. */
export default function ExportBankAccountsDialog({
  open,
  onOpenChange,
  initialStatus = "all",
}: Props) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl border-slate-100 sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <DialogTitle>Export Semua Rekening Bank</DialogTitle>
              <DialogDescription>
                Pilih status dan rentang tanggal rekening ditambahkan.
        ...
        <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
          ...
          <p>
            File berisi nomor rekening lengkap pengguna. Jangan dibagikan tanpa
