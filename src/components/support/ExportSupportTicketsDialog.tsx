...
interface Props {
  ...
  /** Nilai awal mengikuti filter yang aktif di halaman ("all" = semua). */
...
/** Export Tiket Bantuan (docs/be-fe/export-tiket-bantuan.md). Tanggal = tanggal tiket masuk. */
export default function ExportSupportTicketsDialog({
  open,
  onOpenChange,
  initialStatus = "all",
  ...
  useEffect(() => {
    ...
    setQuery("");
    setResults([]);
  }, [open, initialStatus, initialDepartment]);

  useEffect(() => {
    if (!open || reporter) return;
    const q = query.trim();
    if (q.length < 3) {
      setResults([]);
      return;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl border-slate-100 sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <DialogTitle>Export Tiket Bantuan</DialogTitle>
              <DialogDescription>
                Pilih status, departemen, dan rentang tanggal tiket masuk.
                Kosongkan untuk semua tiket.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-2">
          ...
          {reporter ? (
            <div className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
              ...
              <button
                type="button"
                aria-label="Hapus pilihan pelapor"
            ...
            <div className="relative">
              <Input
                placeholder="Cari nama/email (min. 3 huruf), kosong = semua pelapor"
                value={query}
        ...
        <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
          ...
          <p>
            File berisi isi laporan dari pengguna yang bisa memuat data pribadi.
