# Data Export Laporan Mutasi Saldo Semua (Excel, khusus admin)

**Status**: sudah tersedia di backend (2026-09-25). Export ke-12 dari `docs/export/export.md`. Tidak ada template; kolom mengikuti halaman **Laporan Mutasi Saldo Semua** (admin), yaitu riwayat pembayaran semua user (`GET /admin/payments/all`). Versi untuk user (data sendiri): `docs/be-fe/export-laporan-mutasi-saldo.md`.
...
> **Isi export**: catatan pembayaran (tabel `payments`) — pembayaran ongkir order, top up saldo lewat Xendit, dan pendapatan COD. Ini **bukan** mutasi saldo wallet (masuk/keluar dengan saldo sebelum/sesudah); yang itu ada di export Riwayat Dompet (user, data sendiri).
>
> **Nama teknis tidak berubah**: `type` tetap `transactions` dan izinnya tetap `exports.transactions`, walaupun namanya di UI/file sekarang "Laporan Mutasi Saldo Semua". Sebelumnya export ini sempat berlabel "Semua Transaksi"; nama itu sudah tidak dipakai.
...
| `search` | tidak | Maks. 255 karakter. Cari di nomor referensi, external id, invoice id, metode, kanal, serta **nama dan email** pemilik (sama dengan pencarian di `GET /payments/all`) |
...
Tanpa filter → semua transaksi semua akun. Response `202` dan bentuk item export sama dengan dokumen sebelumnya; `type_label` = `Laporan Mutasi Saldo Semua`. Nama file: `laporan-mutasi-saldo-semua[_jenis][_metode][_status][_akun{id}]_{start}_{end}_{waktu}.xlsx`.
...
### Sheet 1 — "Laporan Mutasi Saldo Semua"
